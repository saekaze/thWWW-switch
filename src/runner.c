...
static void applyActiveState(Runner* runner, Instance* instance, bool active) {
    if (instance == nullptr || instance->active == active) return;

#ifdef ENABLE_VM_TRACING
    if (instance->objectIndex >= 0 && (uint32_t) instance->objectIndex < runner->dataWin->objt.count) {
        GameObject* objDef = &runner->dataWin->objt.objects[instance->objectIndex];
        if (shgeti(runner->vmContext->instanceLifecyclesToBeTraced, "*") != -1
            || shgeti(runner->vmContext->instanceLifecyclesToBeTraced, objDef->name) != -1) {
            logInfo("VM: Instance %s (instanceId=%d,objectIndex=%d) marked as %s at (%f, %f)\n",
                objDef->name, instance->instanceId, instance->objectIndex,
                active ? "active" : "inactive", instance->x, instance->y);
        }
    }
#endif

    // Spatial-grid membership follows the committed active state. Reactivation
    // queues insertion before the next collision query, including when a pooled
    // instance returns at unchanged coordinates.
    instance->active = active;
    SpatialGrid_markInstanceAsDirty(runner->spatialGrid, instance);
}

static void flushPendingActiveStates(Runner* runner) {
    int32_t count = (int32_t) arrlen(runner->pendingActiveStateInstances);
    repeat(count, i) {
        Instance* instance = runner->pendingActiveStateInstances[i];
        int8_t pending = instance->pendingActiveState;
        instance->pendingActiveState = -1;
        if (!instance->destroyed && pending >= 0)
            applyActiveState(runner, instance, pending != 0);
    }
    arrsetlen(runner->pendingActiveStateInstances, 0);
}

void Runner_queueActiveState(Runner* runner, Instance* instance, bool active) {
    if (runner == nullptr || instance == nullptr || instance->destroyed) return;

    // Selection by instance_activate/deactivate_* is based on the active state
    // visible at the start of the call. A queued opposite transition must not
    // make the instance selectable again inside the same event.
    if (instance->active == active) return;

    if (runner->eventExecutionDepth == 0) {
        applyActiveState(runner, instance, active);
        return;
    }

    if (instance->pendingActiveState < 0) {
        instance->pendingActiveState = active ? 1 : 0;
        arrput(runner->pendingActiveStateInstances, instance);
    }
}
...
static void Runner_executeResolvedEvent(Runner* runner, Instance* instance, int32_t eventType, int32_t eventSubtype, int32_t codeId, int32_t ownerObjectIndex) {
    ...
    runner->eventExecutionDepth++;
    executeCode(runner, instance, codeId);
    require(runner->eventExecutionDepth > 0);
    runner->eventExecutionDepth--;
    ...
    if (runner->eventExecutionDepth == 0)
        flushPendingActiveStates(runner);
}
...
static int compareDrawKeys(const DrawKey* a, const DrawKey* b) {
    if (a->sortBand != b->sortBand)
        return a->sortBand < b->sortBand ? -1 : 1;

    if (a->depth != b->depth)
        return a->depth > b->depth ? -1 : 1; // higher depth first within a band
...
static void cleanupState(Runner* runner) {
    // Pending activation entries point into the instance arrays freed below.
    arrsetlen(runner->pendingActiveStateInstances, 0);
    runner->eventExecutionDepth = 0;
    ...
    // Free GML byte and vertex buffers.
    {
    repeat(arrlen(runner->gmlBufferPool), i) free(runner->gmlBufferPool[i].data);
    }
    arrfree(runner->gmlBufferPool);
    runner->gmlBufferPool = nullptr;
    {
    repeat(arrlen(runner->gmlVertexBufferPool), i) arrfree(runner->gmlVertexBufferPool[i].vertices);
    }
    arrfree(runner->gmlVertexBufferPool);
    runner->gmlVertexBufferPool = nullptr;
    runner->vertexFormatBuilding = false;
    memset(runner->audioEmitterGains, 0, sizeof(runner->audioEmitterGains));
    memset(runner->audioEmitterActive, 0, sizeof(runner->audioEmitterActive));
    memset(runner->audioEmitterInstanceIds, 0xFF, sizeof(runner->audioEmitterInstanceIds));
    memset(runner->audioEmitterForInstance, 0xFF, sizeof(runner->audioEmitterForInstance));
...
void Runner_free(Runner* runner) {
    ...
    arrfree(runner->pendingActiveStateInstances);
