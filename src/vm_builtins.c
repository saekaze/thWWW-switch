...
static RValue builtin_random_get_seed(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    // Collapse the current xorshift state to a reproducible seed and continue from it. This makes
    // thWWW's stage-boundary replay seed round-trip through random_set_seed exactly.
    uint32_t seed = Random_nextUInt32(&ctx->runner->random);
    Random_setSeed(&ctx->runner->random, seed);
    return RValue_makeReal((GMLReal) seed);
}

static RValue builtin_random(MAYBE_UNUSED VMContext* ctx, RValue* args, int32_t argCount) {
    if (1 > argCount) return RValue_makeReal(0.0);
    GMLReal n = RValue_toReal(args[0]);
    return RValue_makeReal(((GMLReal) Random_nextUInt32(&ctx->runner->random) / (GMLReal) BS_RAND_MAX) * n);
}
...
static RValue builtin_random_range(MAYBE_UNUSED VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    return RValue_makeReal(lo + ((GMLReal) Random_nextUInt32(&ctx->runner->random) / (GMLReal) BS_RAND_MAX) * (hi - lo));
}
...
// GameMaker datetimes are real-valued days relative to 1899-12-30.
static bool gmlDateToLocalTm(GMLReal date, struct tm* out) {
    double unixSeconds = ((double) date - 25569.0) * 86400.0;
    time_t value = (time_t) floor(unixSeconds);
#ifdef _WIN32
...
static RValue builtin_date_current_datetime(MAYBE_UNUSED VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeReal((GMLReal) ((double) time(nullptr) / 86400.0 + 25569.0));
}

#define DEFINE_DATE_PART(name, expression) \
    static RValue builtin_date_get_##name(MAYBE_UNUSED VMContext* ctx, RValue* args, int32_t argCount) { \
        if (1 > argCount) return RValue_makeReal(0.0); \
        struct tm local; \
        if (!gmlDateToLocalTm(RValue_toReal(args[0]), &local)) return RValue_makeReal(0.0); \
        return RValue_makeReal((GMLReal) (expression)); \
    }
DEFINE_DATE_PART(day, local.tm_mday)
DEFINE_DATE_PART(month, local.tm_mon + 1)
DEFINE_DATE_PART(year, local.tm_year + 1900)
DEFINE_DATE_PART(hour, local.tm_hour)
DEFINE_DATE_PART(minute, local.tm_min)
#undef DEFINE_DATE_PART
...
static RValue builtin_ds_list_set(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    int32_t pos = RValue_toInt32(args[1]);
    DsList* list = dsListGet(runner, id);
    if (list == nullptr || pos < 0) return RValue_makeUndefined();
    while ((int32_t) arrlen(list->items) <= pos) arrput(list->items, RValue_makeReal(0.0));
...
// array_concat(array, arrays...) - concatenate each provided array onto a copy of the first array.
static RValue builtin_array_concat(MAYBE_UNUSED VMContext* ctx, RValue* args, int32_t argCount) {
    if (1 > argCount) return RValue_makeArray(GMLArray_create(ctx->dataWin->gen8.wadVersion, 0));
    if (args[0].type != RVALUE_ARRAY || args[0].array == nullptr) return RValue_makeUndefined();

    GMLArray* out = GMLArray_clone(args[0].array, IS_WAD17_OR_HIGHER(ctx) ? ctx->currentArrayOwner : nullptr);
    int32_t outLen = GMLArray_length1D(out);

    for (int32_t i = 1; i < argCount; i++) {
        if (args[i].type != RVALUE_ARRAY || args[i].array == nullptr) {
            GMLArray_decRef(out);
            return RValue_makeUndefined();
        }

        GMLArray* src = args[i].array;
        int32_t addLen = GMLArray_length1D(src);
        if (addLen > 0) {
            GMLArray_growTo(out, outLen + addLen);
            repeat(addLen, j) {
                RValue* slot = GMLArray_slot(out, outLen + j);
                *slot = RValue_makeIndependent(GMLArray_get(src, j));
            }
        }
        outLen += addLen;
    }

    if (IS_WAD17_OR_HIGHER(ctx)) out->owner = ctx->currentArrayOwner;
    return RValue_makeArray(out);
}
...
static RValue builtin_audio_emitter_create(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    Runner* runner = ctx->runner;
    repeat(16, i) {
        if (!runner->audioEmitterActive[i]) {
            runner->audioEmitterActive[i] = true;
            runner->audioEmitterGains[i] = 1.0f;
...
static RValue builtin_audio_emitter_free(VMContext* ctx, RValue* args, int32_t argCount) {
    if (1 > argCount) return RValue_makeUndefined();
    Runner* runner = ctx->runner;
    int32_t emitter = RValue_toInt32(args[0]);
    if (emitter < 0 || emitter >= 16 || !runner->audioEmitterActive[emitter]) return RValue_makeUndefined();

    // Emitter instances are ordinary backend sound instances. Stop and detach
    // all of them before making this emitter slot available for reuse.
    repeat(128, i) {
        if (runner->audioEmitterForInstance[i] != emitter) continue;
        int32_t instance = runner->audioEmitterInstanceIds[i];
        if (instance >= 0) runner->audioSystem->vtable->stopSound(runner->audioSystem, instance);
        runner->audioEmitterInstanceIds[i] = -1;
        runner->audioEmitterForInstance[i] = -1;
    }
    runner->audioEmitterGains[emitter] = 0.0f;
    runner->audioEmitterActive[emitter] = false;
    return RValue_makeUndefined();
}
...
static RValue builtin_audio_emitter_get_gain(VMContext* ctx, RValue* args, int32_t argCount) {
    if (1 > argCount) return RValue_makeReal(0.0);
    int32_t emitter = RValue_toInt32(args[0]);
    if (emitter < 0 || emitter >= 16 || !ctx->runner->audioEmitterActive[emitter]) return RValue_makeReal(0.0);
    return RValue_makeReal((GMLReal) ctx->runner->audioEmitterGains[emitter]);
}
...
static RValue builtin_audio_emitter_gain(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    int32_t emitter = RValue_toInt32(args[0]);
    float gain = (float) RValue_toReal(args[1]);
    uint32_t timeMs = argCount >= 3 ? (uint32_t) RValue_toInt32(args[2]) : 0;
    if (emitter < 0 || emitter >= 16 || !runner->audioEmitterActive[emitter]) return RValue_makeUndefined();
    runner->audioEmitterGains[emitter] = gain;
    repeat(128, i) {
        int32_t instance = runner->audioEmitterInstanceIds[i];
        if (runner->audioEmitterForInstance[i] == emitter && instance >= 0
            && runner->audioSystem->vtable->isPlaying(runner->audioSystem, instance)) {
            runner->audioSystem->vtable->setSoundGain(runner->audioSystem, instance, gain, timeMs);
        }
    }
    return RValue_makeUndefined();
}
...
static RValue builtin_audio_play_sound_on(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    Runner* runner = ctx->runner;
    int32_t emitter = RValue_toInt32(args[0]);
    int32_t sound = RValue_toInt32(args[1]);
    bool loop = RValue_toBool(args[2]);
    int32_t priority = RValue_toInt32(args[3]);
    int32_t instance = runner->audioSystem->vtable->playSound(runner->audioSystem, sound, priority, loop);
    if (instance >= 0 && emitter >= 0 && emitter < 16 && runner->audioEmitterActive[emitter]) {
        runner->audioSystem->vtable->setSoundGain(runner->audioSystem, instance, runner->audioEmitterGains[emitter], 0);
        int32_t slot = -1;
        repeat(128, i) {
            if (runner->audioEmitterInstanceIds[i] < 0
                || !runner->audioSystem->vtable->isPlaying(runner->audioSystem, runner->audioEmitterInstanceIds[i])) {
                slot = (int32_t) i;
                break;
            }
        }
        if (slot >= 0) {
            runner->audioEmitterInstanceIds[slot] = instance;
            runner->audioEmitterForInstance[slot] = (int8_t) emitter;
        }
    }
    return RValue_makeReal((GMLReal) instance);
}

static RValue builtin_audio_create_buffer_sound(VMContext* ctx, RValue* args, int32_t argCount) {
    if (6 > argCount) return RValue_makeReal(-1.0);
    Runner* runner = ctx->runner;
    AudioSystem* audio = runner->audioSystem;
    if (audio == nullptr || audio->vtable->createBufferSound == nullptr) return RValue_makeReal(-1.0);
    GmlBuffer* buffer = gmlBufferGet(runner, RValue_toInt32(args[0]));
    int32_t format = RValue_toInt32(args[1]);
    int32_t sampleRate = RValue_toInt32(args[2]);
    int32_t offset = RValue_toInt32(args[3]);
    int32_t size = RValue_toInt32(args[4]);
    int32_t channels = RValue_toInt32(args[5]) == 0 ? 1 : 2;
    if (buffer == nullptr || offset < 0 || size <= 0 || offset >= buffer->size) return RValue_makeReal(-1.0);
    if (offset + size > buffer->size) size = buffer->size - offset;
    int32_t bits = format == GML_BUFTYPE_U8 ? 8 : 16;
    int32_t sound = audio->vtable->createBufferSound(audio, buffer->data + offset, size, bits, sampleRate, channels);
    return RValue_makeReal((GMLReal) sound);
}

static RValue builtin_audio_free_buffer_sound(VMContext* ctx, RValue* args, int32_t argCount) {
    if (1 > argCount) return RValue_makeBool(false);
    AudioSystem* audio = ctx->runner->audioSystem;
    if (audio == nullptr || audio->vtable->destroyBufferSound == nullptr) return RValue_makeBool(false);
    return RValue_makeBool(audio->vtable->destroyBufferSound(audio, RValue_toInt32(args[0])));
}
...
static RValue builtin_file_text_open_append(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    if (existing == nullptr) existing = safeStrdup("");
    OpenTextFile file = {0};
    file.writeBuffer = existing;
    file.filePath = safeStrdup(path);
    file.contentLen = (int32_t) strlen(existing);
    file.isWriteMode = true;
    file.isOpen = true;
    runner->openTextFiles[slot] = file;
    return RValue_makeReal((GMLReal) slot);
...
static RValue builtin_instance_deactivate_all(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    repeat(instances, i) {
        ...
        if (!notme || instance != ctx->currentInstance) {
            Runner_queueActiveState(ctx->runner, instance, false);
        }
    }
    return RValue_makeUndefined();
}
...
static RValue builtin_instance_activate_all(MAYBE_UNUSED VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    ...
    repeat(instances, i) {
        ...
        if (!instance->destroyed)
            Runner_queueActiveState(ctx->runner, ctx->runner->instances[i], true);
    }
    return RValue_makeUndefined();
}
...
static RValue builtin_instance_activate_object(VMContext* ctx, RValue* args, int32_t argCount) {
    if (1 > argCount) return RValue_makeUndefined();
    Runner* runner = ctx->runner;
    int32_t target = VM_resolveInstanceTarget(ctx, RValue_toInt32(args[0]));
    ...
    for (int32_t i = snapBase; snapEnd > i; i++) {
        ...
        if (!instance->active && !instance->destroyed) Runner_queueActiveState(ctx->runner, instance, true);
    }
    Runner_popInstanceSnapshot(runner, snapBase);
    return RValue_makeUndefined();
}
...
static RValue builtin_instance_deactivate_object(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    for (int32_t i = snapBase; snapEnd > i; i++) {
        Instance* instance = runner->instanceSnapshots[i];
        if (instance->active && !instance->destroyed) Runner_queueActiveState(ctx->runner, instance, false);
    }
    Runner_popInstanceSnapshot(runner, snapBase);
    return RValue_makeUndefined();
}
...
static RValue builtin_instance_activate_region(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    repeat(instances, i) {
        ...
        if (outside != wantInside) Runner_queueActiveState(ctx->runner, inst, true);
    }
    return RValue_makeUndefined();
}
...
static RValue builtin_instance_deactivate_region(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    repeat(instCount, i) {
        ...
        if (outside != wantInside) Runner_queueActiveState(ctx->runner, inst, false);
    }
    return RValue_makeUndefined();
}
...
static RValue builtin_buffer_load_ext(VMContext* ctx, RValue* args, int32_t argCount) {
    if (3 > argCount) return RValue_makeReal(-1.0);
    Runner* runner = ctx->runner;
    int32_t id = RValue_toInt32(args[0]);
    char* filename = RValue_toString(args[1], runner->dataWin);
    int32_t offset = RValue_toInt32(args[2]);
    GmlBuffer* buf = gmlBufferGet(runner, id);
    ...
    uint8_t* fileData = nullptr;
    int32_t fileSize = 0;
    bool ok = runner->fileSystem->vtable->readFileBinary(runner->fileSystem, filename, &fileData, &fileSize);
    free(filename);
    if (!ok) return RValue_makeReal(-1.0);
    gmlBufferEnsureSize(buf, offset + fileSize);
    int32_t copySize = fileSize;
    if (offset >= buf->size) copySize = 0;
    else if (offset + copySize > buf->size) copySize = buf->size - offset;
    if (copySize > 0) memcpy(buf->data + offset, fileData, (size_t) copySize);
    if (offset + copySize > buf->usedSize) buf->usedSize = offset + copySize;
    free(fileData);
    return RValue_makeReal((GMLReal) copySize);
}
...
static RValue builtin_draw_surface_general(VMContext* ctx, RValue* args, MAYBE_UNUSED int32_t argCount) {
    int32_t surfaceId = (int32_t) RValue_toReal(args[0]);
    ...
    uint32_t c1 = RValue_toColour(args[10]);
    uint32_t c2 = RValue_toColour(args[11]);
    uint32_t c3 = RValue_toColour(args[12]);
    uint32_t c4 = RValue_toColour(args[13]);
    float alpha = (float) RValue_toReal(args[14]);


    Runner* runner = ctx->runner;
    if (runner->renderer != nullptr) {
        runner->renderer->vtable->drawSurfaceColor(runner->renderer, surfaceId, left, top, w, h, x, y, xscale, yscale, rot, c1, c2, c3, c4, alpha);
    }
    return RValue_makeUndefined();
}
    ...
    repeat(runner->currentRoom->layerCount, layerIndex) {
        ...
        repeat(layerData->instanceCount, instanceIndex) {
            ...
            if (inst != nullptr && !inst->destroyed)
                Runner_queueActiveState(runner, inst, isActive);
        }
        return;
    }
}
...
static RValue builtin_layer_sprite_angle(VMContext* ctx, RValue* args, MAYBE_UNUSED int32_t argCount) {
    Runner* runner = ctx->runner;
    int32_t id = RValue_toInt32(args[0]);
    RuntimeLayerElement* el = Runner_findLayerElementById(runner, id, nullptr);
    if (isValidLayerSpriteElement(el))
        el->spriteElement->rotation = (float) RValue_toReal(args[1]);
    return RValue_makeUndefined();
}
...
static RValue builtin_layer_sprite_blend(VMContext* ctx, RValue* args, MAYBE_UNUSED int32_t argCount) {
    ...
    RuntimeLayerElement* el = Runner_findLayerElementById(runner, id, nullptr);
    if (isValidLayerSpriteElement(el))
        el->blend = (uint32_t) RValue_toInt32(args[1]);
    return RValue_makeUndefined();
}
...
static RValue builtin_layer_sprite_xscale(VMContext* ctx, RValue* args, MAYBE_UNUSED int32_t argCount) {
    Runner* runner = ctx->runner;
    int32_t id = RValue_toInt32(args[0]);
    RuntimeLayerElement* el = Runner_findLayerElementById(runner, id, nullptr);
    if (isValidLayerSpriteElement(el))
        el->spriteElement->scaleX = RValue_toInt32(args[1]);
    return RValue_makeUndefined();
}

static RValue builtin_layer_sprite_yscale(VMContext* ctx, RValue* args, MAYBE_UNUSED int32_t argCount) {
    Runner* runner = ctx->runner;
    int32_t id = RValue_toInt32(args[0]);
    RuntimeLayerElement* el = Runner_findLayerElementById(runner, id, nullptr);
    if (isValidLayerSpriteElement(el))
        el->spriteElement->scaleY = RValue_toInt32(args[1]);
    return RValue_makeUndefined();
}
...
static void fontCoverageAddUtf8(bool* coverage, const char* text) {
    if (text == nullptr) return;
    int32_t len = (int32_t) strlen(text);
    int32_t pos = 0;
    while (len > pos) {
        uint16_t cp = TextUtils_decodeUtf8(text, len, &pos);
        if (cp >= 0x20 && cp != 0xFFFD) coverage[cp] = true;
    }
}

// font_add(filename, size, bold, italic, first, last)
// GameMaker's native FreeType path can resolve glyphs beyond first..last on demand. Butterscotch
// has no retained-mode font backend, so build a sparse atlas up front from the WAD and thWWW's
// external text files in addition to the requested range.
static RValue builtin_font_add(VMContext* ctx, RValue* args, int32_t argCount) {
    if (6 > argCount || ctx->runner->renderer->vtable->createTextureFromRgba == nullptr) {
        logWarn("[font_add] Expected 6 arguments and a dynamic-texture renderer\n");
        return RValue_makeReal(-1.0);
    }

    char* filename = RValue_toString(args[0], ctx->dataWin);
    float pixelSize = (float) RValue_toReal(args[1]);
    bool bold = RValue_toBool(args[2]);
    bool italic = RValue_toBool(args[3]);
    int32_t first = RValue_toInt32(args[4]);
    int32_t last = RValue_toInt32(args[5]);
    if (pixelSize <= 0.0f) pixelSize = 12.0f;

    uint8_t* ttfData = nullptr;
    int32_t ttfSize = 0;
    if (!ctx->runner->fileSystem->vtable->readFileBinary(ctx->runner->fileSystem, filename, &ttfData, &ttfSize)
        || ttfData == nullptr || ttfSize <= 0) {
        logWarn("[font_add] Could not read '%s'\n", filename);
        free(filename);
        return RValue_makeReal(-1.0);
    }

    stbtt_fontinfo info;
    int32_t fontOffset = stbtt_GetFontOffsetForIndex(ttfData, 0);
    if (fontOffset < 0 || !stbtt_InitFont(&info, ttfData, fontOffset)) {
        logWarn("[font_add] Invalid TTF '%s'\n", filename);
        free(ttfData);
        free(filename);
        return RValue_makeReal(-1.0);
    }

    bool* coverage = (bool*) safeCalloc(65536, sizeof(bool));
    int32_t clampedFirst = first < 0x20 ? 0x20 : first;
    int32_t clampedLast = last > 0xFFFF ? 0xFFFF : last;
    for (int32_t cp = clampedFirst; clampedLast >= cp; cp++) coverage[cp] = true;
    // Always retain printable ASCII for fallback/debug strings.
    for (int32_t cp = 0x20; 0x7E >= cp; cp++) coverage[cp] = true;
    repeat(ctx->dataWin->strg.count, i) fontCoverageAddUtf8(coverage, ctx->dataWin->strg.strings[i]);
    static const char* externalTextFiles[] = {
        "text/GameText_eng.txt", "text/GameText_j.txt", "GameText_eng.txt", "GameText_j.txt",
        "Omake_eng.txt", "Omake_j.txt"
    };
    repeat(sizeof(externalTextFiles) / sizeof(externalTextFiles[0]), i) {
        char* text = ctx->runner->fileSystem->vtable->readFileText(ctx->runner->fileSystem, externalTextFiles[i]);
        if (text != nullptr) {
            fontCoverageAddUtf8(coverage, text);
            free(text);
        }
    }

    uint32_t glyphCount = 0;
    for (uint32_t cp = 0x20; 0x10000 > cp; cp++) {
        if (coverage[cp] && stbtt_FindGlyphIndex(&info, (int32_t) cp) != 0) glyphCount++;
    }
    if (glyphCount == 0) {
        free(coverage);
        free(ttfData);
    ...
    int32_t baseline = (int32_t) ceilf((float) ascent * scale);
    int32_t lineHeight = (int32_t) ceilf((float) (ascent - descent + lineGap) * scale);
    if (lineHeight < 1) lineHeight = (int32_t) ceilf(pixelSize);

    const int32_t atlasWidth = 1024;
    int32_t penX = 1;
    int32_t penY = 1;
    FontGlyph* glyphs = (FontGlyph*) safeCalloc(glyphCount, sizeof(FontGlyph));
    uint32_t glyphIndex = 0;
    for (uint32_t cp = 0x20; 0x10000 > cp; cp++) {
        if (!coverage[cp] || stbtt_FindGlyphIndex(&info, (int32_t) cp) == 0) continue;
        int advance, leftBearing;
        int x0, y0, x1, y1;
        stbtt_GetCodepointHMetrics(&info, (int32_t) cp, &advance, &leftBearing);
        stbtt_GetCodepointBitmapBox(&info, (int32_t) cp, scale, scale, &x0, &y0, &x1, &y1);
        int32_t bitmapWidth = x1 - x0;
        int32_t bitmapHeight = y1 - y0;
        if (bitmapWidth > 0 && penX + bitmapWidth + 1 > atlasWidth) {
            penX = 1;
            penY += lineHeight + 2;
        }

        FontGlyph* glyph = &glyphs[glyphIndex++];
        glyph->character = (uint16_t) cp;
        glyph->sourceX = (uint16_t) penX;
        glyph->sourceY = (uint16_t) penY;
        glyph->sourceWidth = (uint16_t) (bitmapWidth > 0 ? bitmapWidth : 0);
        glyph->sourceHeight = (uint16_t) (bitmapHeight > 0 ? lineHeight : 0);
        glyph->shift = (int16_t) lroundf((float) advance * scale);
        glyph->offset = (int16_t) x0;
        glyph->kerningCount = 0;
        glyph->kerning = nullptr;
        if (bitmapWidth > 0) penX += bitmapWidth + 2;
    }

    int32_t usedHeight = penY + lineHeight + 1;
    int32_t atlasHeight = 64;
    while (atlasHeight < usedHeight && atlasHeight < 4096) atlasHeight *= 2;
    if (atlasHeight < usedHeight) {
        logWarn("[font_add] Sparse atlas for '%s' exceeds 1024x4096\n", filename);
        free(glyphs);
        free(coverage);
        free(ttfData);
        free(filename);
    ...
    uint8_t* alpha = (uint8_t*) safeCalloc((size_t) atlasWidth * atlasHeight, 1);
    repeat(glyphCount, i) {
        FontGlyph* glyph = &glyphs[i];
        if (glyph->sourceWidth == 0 || glyph->sourceHeight == 0) continue;
        int x0, y0, x1, y1;
        stbtt_GetCodepointBitmapBox(&info, glyph->character, scale, scale, &x0, &y0, &x1, &y1);
        int32_t bitmapHeight = y1 - y0;
        int32_t dstY = (int32_t) glyph->sourceY + baseline + y0;
        if (dstY < glyph->sourceY || dstY + bitmapHeight > glyph->sourceY + lineHeight) continue;
        stbtt_MakeCodepointBitmap(&info,
            alpha + (size_t) dstY * atlasWidth + glyph->sourceX,
            glyph->sourceWidth, bitmapHeight, atlasWidth, scale, scale, glyph->character);
    }

    size_t pixelCount = (size_t) atlasWidth * atlasHeight;
    ...
    uint32_t texture = ctx->runner->renderer->vtable->createTextureFromRgba(
        ctx->runner->renderer, rgba, atlasWidth, atlasHeight);
    free(rgba);
    free(alpha);
    free(coverage);
    free(ttfData);
    ...
    ZERO_STRUCT(*font);
    font->present = true;
    font->name = "runtime_font";
    font->displayName = "runtime_font";
    font->emSize = pixelSize;
    font->bold = bold;
    font->italic = italic;
    font->rangeStart = (uint16_t) clampedFirst;
    font->rangeEnd = (uint32_t) clampedLast;
    font->tpagIndex = -1;
    font->scaleX = 1.0f;
    font->scaleY = 1.0f;
    font->ascender = (uint32_t) baseline;
    font->lineHeight = (uint32_t) lineHeight;
    font->hasAscender = true;
    font->hasLineHeight = true;
    font->glyphCount = glyphCount;
    font->glyphs = glyphs;
    font->maxGlyphHeight = (uint32_t) lineHeight;
    font->isSpriteFont = false;
    font->spriteIndex = -1;
    font->runtimeTexture = texture;
    font->runtimeTextureWidth = (uint16_t) atlasWidth;
    font->runtimeTextureHeight = (uint16_t) atlasHeight;
    Font_buildGlyphLUT(font);

    logInfo("[font_add] '%s': %u glyphs, %dx%d atlas\n", filename, glyphCount, atlasWidth, atlasHeight);
    free(filename);
...
static GmlVertexBuffer* gmlVertexBufferGet(Runner* runner, int32_t id) {
    if (id < 0 || id >= (int32_t) arrlen(runner->gmlVertexBufferPool)) return nullptr;
    GmlVertexBuffer* buffer = &runner->gmlVertexBufferPool[id];
    return buffer->isValid ? buffer : nullptr;
}

static void gmlVertexResetPending(GmlVertexBuffer* buffer) {
    ZERO_STRUCT(buffer->pending);
    buffer->pending.r = 255;
    buffer->pending.g = 255;
    buffer->pending.b = 255;
    buffer->pending.a = 255;
    buffer->pendingMask = 0;
}

static void gmlVertexCommitIfComplete(GmlVertexBuffer* buffer) {
    if (buffer->pendingMask == 7) {
        arrput(buffer->vertices, buffer->pending);
        gmlVertexResetPending(buffer);
    }
}

static RValue builtin_vertex_format_begin(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    ctx->runner->vertexFormatBuilding = true;
    return RValue_makeUndefined();
}
...
static RValue builtin_vertex_format_add_color(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    return RValue_makeUndefined();
}
...
static RValue builtin_vertex_format_end(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    ctx->runner->vertexFormatBuilding = false;
    return RValue_makeReal(0.0);
}

static RValue builtin_vertex_create_buffer(VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    Runner* runner = ctx->runner;
    repeat(arrlen(runner->gmlVertexBufferPool), i) {
        if (!runner->gmlVertexBufferPool[i].isValid) {
            GmlVertexBuffer* buffer = &runner->gmlVertexBufferPool[i];
            ZERO_STRUCT(*buffer);
            buffer->isValid = true;
            gmlVertexResetPending(buffer);
            return RValue_makeReal((GMLReal) i);
    ...
    GmlVertexBuffer buffer = {0};
    buffer.isValid = true;
    gmlVertexResetPending(&buffer);
    int32_t id = (int32_t) arrlen(runner->gmlVertexBufferPool);
    arrput(runner->gmlVertexBufferPool, buffer);
    return RValue_makeReal((GMLReal) id);
}

static RValue builtin_vertex_begin(VMContext* ctx, RValue* args, int32_t argCount) {
    if (1 > argCount) return RValue_makeUndefined();
    GmlVertexBuffer* buffer = gmlVertexBufferGet(ctx->runner, RValue_toInt32(args[0]));
    if (buffer == nullptr) return RValue_makeUndefined();
    arrsetlen(buffer->vertices, 0);
...
static RValue builtin_vertex_position_3d(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    buffer->pending.x = (float) RValue_toReal(args[1]);
    buffer->pending.y = (float) RValue_toReal(args[2]);
    buffer->pending.z = (float) RValue_toReal(args[3]);
    buffer->pendingMask |= 1;
    gmlVertexCommitIfComplete(buffer);
    return RValue_makeUndefined();
}

static RValue builtin_vertex_color(VMContext* ctx, RValue* args, int32_t argCount) {
    if (3 > argCount) return RValue_makeUndefined();
    GmlVertexBuffer* buffer = gmlVertexBufferGet(ctx->runner, RValue_toInt32(args[0]));
    if (buffer == nullptr || !buffer->building) return RValue_makeUndefined();
    uint32_t color = (uint32_t) RValue_toInt32(args[1]);
    float alpha = (float) RValue_toReal(args[2]);
    if (alpha < 0.0f) alpha = 0.0f;
    if (alpha > 1.0f) alpha = 1.0f;
    buffer->pending.r = (uint8_t) BGR_R(color);
    buffer->pending.g = (uint8_t) BGR_G(color);
    buffer->pending.b = (uint8_t) BGR_B(color);
    buffer->pending.a = (uint8_t) lroundf(alpha * 255.0f);
    buffer->pendingMask |= 2;
    gmlVertexCommitIfComplete(buffer);
    return RValue_makeUndefined();
}
...
static RValue builtin_vertex_texcoord(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    GmlVertexBuffer* buffer = gmlVertexBufferGet(ctx->runner, RValue_toInt32(args[0]));
    if (buffer == nullptr || !buffer->building) return RValue_makeUndefined();
    buffer->pending.u = (float) RValue_toReal(args[1]);
    buffer->pending.v = (float) RValue_toReal(args[2]);
    buffer->pendingMask |= 4;
    gmlVertexCommitIfComplete(buffer);
    return RValue_makeUndefined();
}
...
static RValue builtin_vertex_end(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    GmlVertexBuffer* buffer = gmlVertexBufferGet(ctx->runner, RValue_toInt32(args[0]));
    if (buffer != nullptr) {
        if (buffer->pendingMask != 0) arrput(buffer->vertices, buffer->pending);
...
static RValue builtin_vertex_submit(VMContext* ctx, RValue* args, int32_t argCount) {
    if (2 > argCount) return RValue_makeUndefined();
    GmlVertexBuffer* buffer = gmlVertexBufferGet(ctx->runner, RValue_toInt32(args[0]));
    if (buffer == nullptr || ctx->runner->renderer->vtable->drawVertexBuffer == nullptr) return RValue_makeUndefined();
    int32_t primitive = RValue_toInt32(args[1]);
    uint32_t texture = argCount >= 3 ? (uint32_t) RValue_toInt32(args[2]) : 0;
    ctx->runner->renderer->vtable->drawVertexBuffer(ctx->runner->renderer, buffer->vertices,
        (int32_t) arrlen(buffer->vertices), primitive, texture);
    return RValue_makeUndefined();
}
...
static RValue builtin_gpu_set_ztestenable(VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount >= 1 && ctx->runner->renderer->vtable->gpuSetZTestEnable != nullptr)
        ctx->runner->renderer->vtable->gpuSetZTestEnable(ctx->runner->renderer, RValue_toBool(args[0]));
    return RValue_makeUndefined();
}
...
void VMBuiltins_registerAll(VMContext* ctx) {
    ...
    VM_registerBuiltin(ctx, "array_concat", builtin_array_concat);
    ...
    VM_registerBuiltin(ctx, "audio_create_buffer_sound", builtin_audio_create_buffer_sound);
    VM_registerBuiltin(ctx, "audio_free_buffer_sound", builtin_audio_free_buffer_sound);
    ...
    VM_registerBuiltin(ctx, "layer_sprite_xscale", builtin_layer_sprite_xscale);
    ...
    VM_registerBuiltin(ctx, "font_add", builtin_font_add);
    ...
    VM_registerBuiltin(ctx, "vertex_color", builtin_vertex_color);
