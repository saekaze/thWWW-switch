...
// Interleaved format used by GameMaker's runtime vertex buffers.
typedef struct {
    float x, y, z;
    float u, v;
    uint8_t r, g, b, a;
...
typedef struct {
    ...
    // Draws an interleaved GameMaker vertex buffer. Primitive values use the
    // GameMaker pr_* constants (thWWW uses pr_trianglelist == 4).
    void (*drawVertexBuffer)(Renderer* renderer, const RendererVertex* vertices, int32_t vertexCount, int32_t primitive, uint32_t texture);
    // Creates a renderer-owned RGBA texture for font_add runtime atlases.
    uint32_t (*createTextureFromRgba)(Renderer* renderer, const uint8_t* rgba, int32_t width, int32_t height);
    ...
    void (*gpuSetZTestEnable)(Renderer* renderer, bool enable);
