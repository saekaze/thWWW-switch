...
typedef struct {
    ...
    // font_add() fonts are rasterized after renderer initialization, so their
    // atlas is a renderer-owned texture rather than a TXTR/TPAG resource.
    uint32_t runtimeTexture;
    uint16_t runtimeTextureWidth;
    uint16_t runtimeTextureHeight;
} Font;
