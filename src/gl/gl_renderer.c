...
static bool glResolveTextureHandle(GLRenderer* gl, uint32_t texHandle, TexturePageItem** outTpag,
                                   GLuint* outTexId, int32_t* outTexW, int32_t* outTexH);

static void glDrawVertexBuffer(Renderer* renderer, const RendererVertex* vertices, int32_t vertexCount, int32_t primitive, uint32_t texture) {
    ...
    if (renderer->currentShader != -1) {
        ...
        if (shader->gmBaseTexture != nullptr)
            glActiveTexture(GL_TEXTURE0 + shader->gmBaseTexture->samplerSlot);
    } else {
        glActiveTexture(GL_TEXTURE1);
    }
    GLuint textureId = gl->whiteTexture;
    if (texture != 0) {
        TexturePageItem* ignoredTpag = nullptr;
        int32_t ignoredWidth = 0, ignoredHeight = 0;
        if (!glResolveTextureHandle(gl, texture, &ignoredTpag, &textureId, &ignoredWidth, &ignoredHeight)) {
            logWarn("GL: vertex_submit received invalid texture handle %u\n", texture);
            return;
        }
    }
    ...
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 4, GL_UNSIGNED_BYTE, GL_TRUE, stride, (void*) offsetof(RendererVertex, r));
    glEnableVertexAttribArray(1);
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, stride, (void*) offsetof(RendererVertex, u));
    glEnableVertexAttribArray(2);
...
static uint32_t glCreateTextureFromRgba(Renderer* renderer, const uint8_t* rgba, int32_t width, int32_t height) {
    GLRenderer* gl = (GLRenderer*) renderer;
    ...
    arrput(gl->runtimeTextures, texture);
    gl->currentTextureId = 0;
...
static void glDestroy(Renderer* renderer) {
    ...
    if (arrlen(gl->runtimeTextures) > 0)
        glDeleteTextures((GLsizei) arrlen(gl->runtimeTextures), gl->runtimeTextures);
    arrfree(gl->runtimeTextures);
    ...
    repeat(gl->surfaceCount, i) {
        ...
        if (gl->surfaceDepth != nullptr && gl->surfaceDepth[i] != 0) glDeleteRenderbuffers(1, &gl->surfaceDepth[i]);
...
static bool glResolveFontState(GLRenderer* gl, DataWin* dw, Font* font, GlFontState* state) {
    ...
    if (!font->isSpriteFont) {
        if (font->runtimeTexture != 0) {
            state->texId = (GLuint) font->runtimeTexture;
            state->texW = font->runtimeTextureWidth;
            state->texH = font->runtimeTextureHeight;
        } else {
...
static bool glResolveGlyph(GLRenderer* gl, DataWin* dw, GlFontState* state, FontGlyph* glyph, float cursorX, float cursorY, GLuint* outTexId, float* outU0, float* outV0, float* outU1, float* outV1, float* outLocalX0, float* outLocalY0) {
    ...
    if (font->isSpriteFont && state->spriteFontSprite != nullptr) {
        ...
        *outTexId = state->texId;
        int32_t pageX = state->fontTpag != nullptr ? state->fontTpag->sourceX : 0;
        int32_t pageY = state->fontTpag != nullptr ? state->fontTpag->sourceY : 0;
        *outU0 = (float) (pageX + glyph->sourceX) / (float) state->texW;
        *outV0 = (float) (pageY + glyph->sourceY) / (float) state->texH;
        *outU1 = (float) (pageX + glyph->sourceX + glyph->sourceWidth) / (float) state->texW;
        *outV1 = (float) (pageY + glyph->sourceY + glyph->sourceHeight) / (float) state->texH;
...
static int32_t glCreateSurface(Renderer* renderer, int32_t width, int32_t height) {
    ...
    uint32_t previousSurfaceCount = gl->surfaceCount;
    ...
    if (gl->surfaceCount != previousSurfaceCount) {
        gl->surfaceDepth = (GLuint*) safeRealloc(gl->surfaceDepth, gl->surfaceCount * sizeof(GLuint));
        memset(gl->surfaceDepth + previousSurfaceCount, 0, (gl->surfaceCount - previousSurfaceCount) * sizeof(GLuint));
    }
    ...
    glGenRenderbuffers(1, &gl->surfaceDepth[surfaceIndex]);
    glBindRenderbuffer(GL_RENDERBUFFER, gl->surfaceDepth[surfaceIndex]);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, width, height);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, gl->surfaceDepth[surfaceIndex]);
...
static void glSurfaceFree(Renderer* renderer, int32_t surfaceID) {
    ...
    if (gl->surfaceDepth[surfaceID] != 0) glDeleteRenderbuffers(1, &gl->surfaceDepth[surfaceID]);
...
static void glSurfaceResize(Renderer* renderer, int32_t surfaceID, int32_t width, int32_t height) {
    ...
    glGenRenderbuffers(1, &gl->surfaceDepth[surfaceID]);
    glBindRenderbuffer(GL_RENDERBUFFER, gl->surfaceDepth[surfaceID]);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, width, height);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, gl->surfaceDepth[surfaceID]);
...
static void glGpuSetZTestEnable(Renderer* renderer, bool enable) {
    GLRenderer* gl = (GLRenderer*) renderer;
    flushBatch(gl);
    if (enable) {
...
Renderer* GLRenderer_create(void) {
    ...
    glVtable.createTextureFromRgba = glCreateTextureFromRgba;
    ...
    glVtable.gpuSetZTestEnable = glGpuSetZTestEnable;
