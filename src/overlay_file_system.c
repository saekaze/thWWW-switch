...
static char* resolveForWrite(OverlayFileSystem* ofs, const char* relativePath) {
    ...
    // Paths explicitly rooted in the save area stay there. GameMaker games
    // often build writable paths by prefixing working_directory (the bundle
    // root); redirect those back into the save overlay instead of modifying
    // files beside data.win. These checks must precede the generic absolute-
    // path case because working_directory is absolute on Switch.
    size_t saveLen = strlen(ofs->savePath);
    if (strncmp(normalized, ofs->savePath, saveLen) == 0) return normalized;
    size_t bundleLen = strlen(ofs->bundlePath);
    if (strncmp(normalized, ofs->bundlePath, bundleLen) == 0) {
        char* full = joinPath(ofs->savePath, normalized + bundleLen);
        free(normalized);
        return full;
    }
    if (isAbsolute(normalized)) return normalized;
