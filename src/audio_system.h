...
typedef struct {
    ...
    // Runtime PCM resources backing audio_create_buffer_sound.
    int32_t (*createBufferSound)(AudioSystem* audio, const uint8_t* data, int32_t size, int32_t bitsPerSample, int32_t sampleRate, int32_t channels);
    bool (*destroyBufferSound)(AudioSystem* audio, int32_t bufferSoundIndex);
} AudioSystemVtable;
