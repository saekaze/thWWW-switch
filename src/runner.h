...
// Runtime vertex buffers used by vertex_* builtins.
typedef struct {
    RendererVertex* vertices; // stb_ds array
    RendererVertex pending;
    uint8_t pendingMask;      // position/color/texcoord bits
    bool isValid;
    bool building;
} GmlVertexBuffer;
...
struct Runner {
    ...
    // GameMaker queues instance activation/deactivation until the current event
    // returns. Nested events (for example a Destroy event called from Step) are
    // part of the outer dispatch and flush together.
    Instance** pendingActiveStateInstances;
    ...
    GmlVertexBuffer* gmlVertexBufferPool; // stb_ds array of GmlVertexBuffer
    bool vertexFormatBuilding;
    // thWWW uses emitters for independent BGM/SFX volume controls only.
    float audioEmitterGains[16];
    bool audioEmitterActive[16];
    int32_t audioEmitterInstanceIds[128];
    int8_t audioEmitterForInstance[128];
...
// Queue an instance active-state change. GameMaker does not expose activation
// or deactivation to instance iteration until the current event has finished.
// Calls made outside an event are applied immediately.
